...
# --- 2. KULLANICI VERİTABANI (ŞİFRELENMİŞ HASH İLE) ---
# Yeni müşteri eklerken şifresini admin panelindeki "Şifre Üretici" den dönüştürüp buraya yazmalısın.
MUSTERILER = {
    "giray_admin": {
        "sifre": "c94e0988cc26e5fc5f27c388db421d374ce7a58ed52b144bc306cd1769ee385c", # patron123
        "bitis_tarihi": "2030-12-31" 
    },
    "ilk_musteri": {
        "sifre": "8156cd4e2b0c36cb20c6aeb8519bc8925562725ec3ed8521a2c3a5efc272a276", # deneme20
...
def login_ekrani():
    ...
    with st.form("login_form"):
        ...
        if submit_button:
            # Girilen şifreyi anında güvenlik koduna (hash) çeviriyoruz
            girilen_hash = hashlib.sha256(sifre.encode()).hexdigest()
            
            if kullanici_adi in MUSTERILER and MUSTERILER[kullanici_adi]["sifre"] == girilen_hash:
                bitis_str = MUSTERILER[kullanici_adi]["bitis_tarihi"]
                bitis_tarihi = datetime.datetime.strptime(bitis_str, "%Y-%m-%d").date()
                bugun = datetime.date.today() # Sunucu saati (Güvenlidir, kullanıcı değiştiremez)
...
def ana_uygulama():
    ...
    # Sadece sana özel Şifre Üretici Aracı
    if st.session_state["kullanici"] == "giray_admin":
        with st.sidebar.expander("🔐 Yeni Müşteri Şifre Üretici"):
            st.write("Yeni müşteriye vereceğiniz şifreyi yazın, sisteme ekleyeceğiniz kod aşağıda çıksın.")
            yeni_sifre = st.text_input("Şifre Yazın:", type="password", key="new_pass")
            if yeni_sifre:
                st.code(hashlib.sha256(yeni_sifre.encode()).hexdigest())
        st.sidebar.markdown("---")
    ...
    st.markdown("API'de olmayan amatör ligleri, kupa maçlarını veya **'Peki ya şöyle olsaydı?'** dediğin varsayımsal senaryoları burada özgürce test et.")
    st.markdown("---")
    ...
    if submit:
        # HATA KONTROLLERİ (Mantık Filtresi)
        if (h_w + h_d) > h_m:
        ...
        with st.spinner("Laboratuvar Verileri İşleniyor..."):
            ai = MatchAnalyzer()
            
            h_att = ai.attack_score(h_gf, h_m)
            h_def = ai.defence_score(h_ga, h_m)
            
            # Form hesabı için kayıplar otomatik hesaplanıyor
            h_loss = h_m - h_w - h_d
            ...
            # Form skorlarının ilerleyen aşamada expected_goals içine gönderilmesi gerekiyor (Ai güncellenecek)
            home_xg = ai.expected_goals(h_att, a_def)
            away_xg = ai.expected_goals(a_att, h_def)
            ...
            st.markdown("---")
            st.header("📈 Derin Analiz Çıktıları")
